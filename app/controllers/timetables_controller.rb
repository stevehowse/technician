class TimetablesController < ApplicationController
  ...
  def index
	...
	def update_all
	params['timetable'].keys.each do |id|
    @timetable = Timetable.find(id.to_i)
	@timetable.update_attributes(params['timetable'][id])
  end
  session[:return_to] ||= request.referer
redirect_to user_timetable_path(:week=>@timetable.week)
	end
	
	def timetable
		@user = User.where("id = ?", current_user.id).first
		@periods = School.where("id = ?", @user.school_id).first
